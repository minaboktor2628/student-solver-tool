...
- Make sure to add the `.env` file. Follow the instructions in the example env file to see what variables need to be set.
  In production, the `DATABASE_URL` variable <u>**needs**</u> to be `file:/data/prod.db`.
  Therefor, it is manually set in the `docker-compose.yml` file. Setting it to anything differently in the .env file will be ignored.
...
sudo usermod -aG deploysts <your username>
```
...
> [!WARNING]
> This will replay your current dev.db if it exists. Only run this command if your are absolutely sure.
